...
def generate_makefile_mk(directory, make_relative_path, default_args):  
    floder_path = os.path.abspath(directory)
    make_path = os.path.join(floder_path, './build') #.replace('\./', '/')
    print(make_path)
...
if __name__ == "__main__":  
    ...
    root_path = args.r
    build_path = os.path.join(os.path.abspath(root_path), 'build')
    ...
    if line_number is not None:  
        ...
        gcc_flags_path = os.path.join(build_path, 'S32K3_M7_0_0.args')         
        with open(gcc_flags_path, 'w') as file:  
            file.writelines(link_lines)  
            file.close()
    # os.system('cp ./config/compile.mk ./build/')
    subprocess.run(["powershell", 'cp ./config/compile.mk {0}'.format(build_path)], capture_output=True, text=True)
